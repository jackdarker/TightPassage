...
class LineOfSight():
    ...
    obstacles is a list of rects
    returned value is Vector2 that represents the view-line or None if not possible

    draw viewline from source to target
    for each obstacle:
    draw additional lines from source to each corner of the obstacle-rect
    calculate angle difference between viewline and each obstacle line
    if at least one difference has a different sign then the others, the viewline is blocked by the obstacle
    (view line in cone created by obstacle lines)
    """
